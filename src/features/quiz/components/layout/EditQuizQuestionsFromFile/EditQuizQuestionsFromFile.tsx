...
const EditQuizQuestionsFromFile: FC<EditQuizQuestionsFromFileProps> = () => {
	...
	async function handleGenerateQuestions(e: FormEvent<HTMLFormElement>) {
		...
		try {
			// Validation
			validateQuizGenerationData(file, randomStrategy, questionCount, answerOptionCount);
		} catch (err) {
			console.log(err);
			return;
		}
		...
		try {
			...
			// Generate questions
			await generateQuestions(quiz.id, file as File, strategy, questionCount, answerOptionCount);
		} catch (err) {
